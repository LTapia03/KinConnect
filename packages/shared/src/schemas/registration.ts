...
const validateNightsSelection = (
  data: { nightsStaying: unknown[]; nightsOther: string | null | undefined },
  ctx: z.RefinementCtx,
) => {
...
export const RegistrationInsertSchema =
  RegistrationFieldsSchema.strict().superRefine(validateNightsSelection);

export type RegistrationInsertInput = z.infer<typeof RegistrationInsertSchema>;

export const RegistrationSchema = RegistrationFieldsSchema.extend({
  status: RegistrationStatusSchema,
}).superRefine(validateNightsSelection);
